...
주제어 하나만 입력하면 네이버 블로그 **홈판(메인 피드)** 노출을 겨냥한 완성글과 대표
썸네일 프롬프트를 AI가 한 번에 만들어주는 서비스입니다. 분야별로 별도 제작된 "0~7단계"
작성 가이드(`prompts/*.md`)를 카테고리로 선택하면, Claude가 실시간 웹 검색으로 정보를
수집하고, 제목 전략을 적용하고, 팩트체크까지 마친 뒤 **바로 붙여넣을 수 있는 완성글 +
메인 썸네일 프롬프트 1개**만 돌려줍니다.
...
- 주제어 + 카테고리(9종, 분야별 전용 프롬프트) 선택만으로 완성글 생성
- 생성 시 Claude의 웹 검색 도구를 사용해 실제 최신 정보를 조사하고 팩트체크 (`prompts/*.md`의
  1단계·5단계 요구사항 반영) — 생성에 다소 시간이 걸리고 비용이 늘어날 수 있습니다
- 결과는 하나의 완성글(제목+소제목+본문+해시태그 10개, 그대로 복사 가능)과 메인 썸네일
  프롬프트 1개로 단순화 (분야별 가이드의 30개 제목·소제목별 썸네일 등 중간 산출물은 내부
  처리만 하고 노출하지 않음)
- 톤 조절: 더 자극적으로 / 더 전문적으로 / 더 자연스럽게 / 짧게 / 길게, 메인 썸네일 다시 만들기
- 전체 다시 생성, 복사, 초기화
...
- `prompts/*.md` — 분야별 "0~7단계" 네이버 블로그 홈판 작성 가이드 (사용자 제공, 카테고리별 시스템 프롬프트 원문)
- `lib/categories.ts` — 카테고리 ID ↔ 표시 이름 ↔ 프롬프트 파일 매핑
- `lib/systemPrompt.ts` — 위 가이드를 "단일 호출로 0~5단계 + 7단계 메인 썸네일만 반환"하도록
  감싸는 실행 모드 안내문(절대 원칙 포함)을 붙여 최종 시스템 프롬프트를 구성
- `lib/contentSchema.ts` — 최종 결과용 `output_post` 도구 스키마와 웹 검색 서버 도구 정의
- `lib/actions.ts` — "더 자극적으로", "메인 썸네일 다시 만들기" 등 각 액션의 지시문
- `app/api/generate/route.ts` — 생성/재생성 요청을 처리하는 API 라우트 (웹 검색 + output_post 도구 사용)
